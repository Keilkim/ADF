...
지원이 승인되고 실제 연동이 완료되면 SignPath가 요구하는 제공자·인증서 고지를 추가합니다. 승인 전에는 SignPath가 ADF를 서명하거나 보증하는 것으로 표시하지 않습니다. 현재 빌드 방식은 공개 PowerShell/Python 스크립트를 이용하는 로컬 Windows 빌드이며, GitHub Actions 빌드나 SignPath 연동이 이미 운영 중이라고 기재하지 않습니다.
