...
- ADF 0.3.32: Windows는 미서명. macOS는 0.3.30 앱과 DMG가 Developer ID 서명·Apple 공증 및 Gatekeeper 검증 완료.
- macOS 소스 검사 386개와 세부 검사 129개, 앱 내부 기능 88개 항목, PDF 작업 프로세스와 패키지 검사 통과. Finder 설치·실행도 확인. PDF 기본 연결·실제 입력 장치·인쇄 조작·macOS 15 실기기·다른 Mac 설치는 추가 확인 필요. [Mac 0.3.30 검증 기록](docs/검증-0.3.30-macOS.md).
- Windows 소스 검사: 403개와 세부 검사 136개, 탐색기 확장 구성 요소 검사 14,567개 통과. Windows Server 2022에서 설치·파일 대조·바로가기·앱 실행·문서 처리·제거 확인.
...
2026년 9월 10일 SignPath Foundation에 무료 오픈소스 서명 지원을 신청했으나, 9월에 공개 인지도(GitHub 스타·포크·기여자, 외부 글과 언급 등)가 아직 부족하다는 이유로 승인되지 않았습니다. 인지도가 쌓이면 다시 신청할 수 있습니다. 그 전까지는 유료 코드 서명 인증서를 검토하며, 실제 서명은 아직 완료되지 않았습니다. 스마트 앱 컨트롤은 신뢰된 인증 기관의 서명이 없는 새 파일을 막으므로, 서명 전까지 설치한 ADF의 자동 업데이트는 스마트 앱 컨트롤이 켜진 PC에서 자동 설치 대신 다운로드 페이지를 안내합니다.
